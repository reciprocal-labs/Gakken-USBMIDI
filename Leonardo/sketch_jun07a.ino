...
void handle_noteOn(byte pitch, byte velocity) {
...
void handle_noteOff(byte pitch, byte velocity) {
...
void handle_pitchBend(int bend) {
...
void handle_glideParamUpdate(byte number, byte value){
...
void handle_CC(byte number, byte value){
  
  switch(number){
    case 1: // MIDI CC #1 is the Mod Wheel
      handle_glideParamUpdate(number, value);
      break;
...
void loop() {
  ...
  do {
    ...
    case 0x09: // note on
      handle_noteOn(packet.byte2, packet.byte3);
      break;
    case 0x08: // note off
      handle_noteOff(packet.byte2, packet.byte3);
      break;
    case 0x0E: // pitch bend message
      bend = (packet.byte3 * 128) + packet.byte2;
      handle_pitchBend(bend);
      break;
    case 0x0B: // control change
      handle_CC(packet.byte2, packet.byte3);
      break;
    }
